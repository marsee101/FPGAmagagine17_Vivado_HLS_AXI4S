...
#pragma HLS INTERFACE s_axilite port=return
	...
	unsigned int line_buf[2][HORIZONTAL_PIXEL_WIDTH+1];
...
#pragma HLS PIPELINE II=1
	        if ((!(x==0 && y==0)) && x<HORIZONTAL_PIXEL_WIDTH && y<VERTICAL_PIXEL_WIDTH)
	        	// 最初の入力はすでに入力されている。最後の列と最後の行はデータが無いので読み込まない
			...
			if (x<2 || y<2 || x==HORIZONTAL_PIXEL_WIDTH || y==VERTICAL_PIXEL_WIDTH)
			// 最初の2行+最後の行とその他の行の最初の2列+最後の列は無効データなので0とする
				lap.data = 0;

			if (x==1 && y==1) // 最初のデータでは、TUSERをアサートする
				lap.user = 1;
			else
				lap.user = 0;
			
			if (x == (HORIZONTAL_PIXEL_WIDTH))	// 行の最後で TLAST をアサートする
				lap.last = 1;
			else
				lap.last = 0;

			if (x>0 && y>0) // 最初の列と行は出力しない
