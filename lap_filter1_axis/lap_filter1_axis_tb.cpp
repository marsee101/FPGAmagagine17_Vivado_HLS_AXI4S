// lap_filter1_axis_tb.cpp
// 2015/05/01
// 2015/08/17 : BMPファイルを読み書きするように変更した
// 2017/02/18 : ラプラシアンフィルタ画像の周りを1ピクセル 0 にした
//
...
int lap_filter_axis_soft(hls::stream<ap_axis<32,1,1,1> >& ins, hls::stream<ap_axis<32,1,1,1> >& outs, int width, int height){
    ...
    // メモリをアロケートする
    for (i=0; i<2; i++){
        if ((line_buf[i]=(unsigned int *)malloc(sizeof(unsigned int) * (width+1))) == NULL){
    ...
    for (int y=0; y<=height; y++){
        for (int x=0; x<=width; x++){
            if ((!(x==0 && y==0)) && x<width && y<height)    // 最初の入力はすでに入力されている
            	// 最後の列と最後の行の時はすでに入力されているので入力する必要はない
            ...
            if (x<2 || y<2 || x==width || y==height)
            // 最初の2行とその他の行の最初の2列、最後の列と最後の行は無効データなので0とする
                lap.data = 0;
            ...
            if (x == width)    // 行の最後で TLAST をアサートする
                lap.last = 1;
            else
                lap.last = 0;
